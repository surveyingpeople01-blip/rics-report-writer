import { Zap, ChevronDown, ChevronUp, Image as ImageIcon, MessageCircle, Plus } from 'lucide-react';
...
// Section F Data interface
interface SectionFData {
    limitationsText: string;
    f1Electricity: SubsectionData;
    f2GasOil: SubsectionData;
    f3Water: SubsectionData;
    f4Heating: SubsectionData;
    f5WaterHeating: SubsectionData;
    f6Drainage: SubsectionData;
    f7CommonServices: SubsectionData;
}

interface SectionFProps {
    data: SectionFData;
    onDataChange: (data: Partial<SectionFData>) => void;
...
// Default Section F data
const defaultSectionFData: SectionFData = {
    limitationsText: `A visual non-invasive inspection of the services was carried out. Specialist tests were not conducted but services were checked through their normal operation in everyday use. If any services (such as the boiler or mains water) were turned off, they were not turned on for safety reasons and the report will state that to be the case.

The reports only comments on the services covered in this section (electricity, gas, oil, water, heating and drainage). All other services and domestic appliances are not included in the inspection: for example security and door answering systems, smoke alarms, television, cable, wireless and satellite communication systems, cookers, hobs, washing machines and fridges (even where built in).`,
    f1Electricity: { ...defaultSubsection },
    f2GasOil: { ...defaultSubsection },
    f3Water: { ...defaultSubsection },
    f4Heating: { ...defaultSubsection },
    f5WaterHeating: { ...defaultSubsection },
    f6Drainage: { ...defaultSubsection },
    f7CommonServices: { ...defaultSubsection }
};

export function SectionF({ data, onDataChange, autoResponses, onAddPhoto }: SectionFProps) {
    const subsections = [
        { key: 'f1Electricity', id: 'f1', title: 'F1 Electricity' },
        { key: 'f2GasOil', id: 'f2', title: 'F2 Gas/oil' },
        { key: 'f3Water', id: 'f3', title: 'F3 Water' },
        { key: 'f4Heating', id: 'f4', title: 'F4 Heating' },
        { key: 'f5WaterHeating', id: 'f5', title: 'F5 Water heating' },
        { key: 'f6Drainage', id: 'f6', title: 'F6 Drainage' },
        { key: 'f7CommonServices', id: 'f7', title: 'F7 Common services' },
    ];
                ...
                <div className="space-y-4">
                    {subsections.map(({ key, id, title }) => (
                        <Subsection
                            ...
                            id={id}
                            title={title}
                            data={data[key as keyof SectionFData] as SubsectionData}
...
export { defaultSectionFData };
export type { SectionFData };
