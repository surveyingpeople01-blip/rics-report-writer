import { FileText, Trash2, Clock, User, Grid, List as ListIcon, ChevronDown, Layers, Search, LogOut } from 'lucide-react';
import { useState } from 'react';
import logoImg from '/logo.jpg';

export type ReportStatus = 'working' | 'complete' | 'archived';
export type ReportType = 'level2' | 'level3';
...
interface DashboardProps {
    reports: SavedReport[];
    onNewReport: (type: ReportType) => void;
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col">
                    ...
                    <div className="flex-1 flex justify-center">
                        <div className="text-left">
                            ...
                            <p className="text-[#d4b88a] text-base md:text-lg mt-1">
                                Professional Home Survey Report System - London, UK
                            </p>
                        </div>
                    </div>
            ...
            <main className="flex-1 pt-0 pb-8 px-8 max-w-6xl mx-auto w-full">
                ...
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-[32px] font-bold text-[#002855]">RICS Home Survey Reports</h2>
                    <div className="relative">
                        ...
                        {showCreateMenu && (
                            <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-200 py-2 z-50">
                                <button
                                    onClick={() => { onNewReport('level2'); setShowCreateMenu(false); }}
                                    className="w-full text-left px-4 py-3 hover:bg-gray-50 flex flex-col gap-0.5"
                                >
                                    <span className="font-bold text-gray-900">RICS Level 2 Survey</span>
                                    <span className="text-xs text-gray-500">Essential inspection for modern homes</span>
                                </button>
                                <div className="h-px bg-gray-100 mx-2" />
                                <button
                                    onClick={() => { onNewReport('level3'); setShowCreateMenu(false); }}
                                    className="w-full text-left px-4 py-3 hover:bg-gray-50 flex flex-col gap-0.5"
                                >
                                    <span className="font-bold text-gray-900">RICS Level 3 Survey</span>
                                    <span className="text-xs text-gray-500">Comprehensive structural assessment</span>
                                </button>
                            </div>
                        )}
                    </div>
                ...
                {filteredReports.length === 0 ? (
                    <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-20 text-center flex flex-col items-center justify-center space-y-4">
                        ...
                        <p className="text-gray-500 max-w-xs mx-auto text-lg text-center">
                            {activeTab === 'all'
                                ? "Start by creating your first survey report for a client."
                                        ...
                                        <tr key={report.id} className="hover:bg-gray-50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-1 text-[10px] font-bold text-gray-500 uppercase">
                                                    <Layers size={12} className="text-survey-blue" />
                                                    {report.type === 'level2' ? 'Level 2' : 'Level 3'}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <StatusSelector
            ...
            <div className="p-6 flex-1">
                <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex items-center gap-1 text-[10px] font-bold text-gray-400 uppercase">
                        <Layers size={14} className="text-survey-blue/50" />
                        {report.type === 'level2' ? 'Level 2' : 'Level 3'}
                    </div>
                </div>
